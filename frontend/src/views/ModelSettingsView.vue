      ...
      <main class="settings-main">
        <div class="settings-grid">
          <section class="panel panel-pad settings-panel">
            <div class="panel-heading"><div><p class="section-kicker">Text reasoning</p><h2>文字推理</h2><p>分析、规划、Prompt、评审、文案和修改意图均要求真实 LLM。</p></div></div>
            ...
            <el-form v-else label-position="top">
              ...
              <template v-if="form.text_provider"><el-form-item label="文字模型"><el-input v-model="form.providers[form.text_provider].text_model" placeholder="填写该平台的文字模型 ID" /></el-form-item><el-form-item label="Base URL"><el-input v-model="form.providers[form.text_provider].base_url" /></el-form-item></template>
            </el-form>
          </section>
...
const form = reactive<{ text_provider: string; image_provider: string; providers: Record<string, ProviderModelSettings> }>({
  ...
  providers: {
    openai: { text_model: '', image_model: '', base_url: '', api_key_configured: false },
    dashscope: { text_model: '', image_model: '', base_url: '', api_key_configured: false }
  }
})
...
<style scoped>
.model-page { max-width: 1280px; }.settings-header { align-items: flex-end; }.settings-alert { margin-bottom: 12px; }.settings-actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 8px; }.settings-shell { display: grid; grid-template-columns: 260px minmax(0, 1fr); gap: 14px; align-items: start; }.settings-rail { position: sticky; top: 22px; display: grid; gap: 14px; }.status-stack, .settings-main { display: grid; gap: 10px; }.status-row { display: grid; grid-template-columns: 10px minmax(0, 1fr); gap: 10px; padding: 10px; border: 1px solid var(--ps-border); border-radius: var(--ps-radius); background: var(--ps-surface-quiet); }.status-row strong, .status-row small { display: block; }.status-row small, .panel-heading p, dd { color: var(--ps-muted); }.status-row small { margin-top: 4px; font-size: 12px; line-height: 1.45; }.settings-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }.panel-heading { display: flex; justify-content: space-between; gap: 14px; margin-bottom: 16px; }.panel-heading h2 { margin: 0; color: var(--ps-heading); font-size: 20px; }.panel-heading p { margin: 7px 0 0; line-height: 1.65; }.test-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; }dt { color: var(--ps-primary); font-size: 12px; font-weight: 800; }dd { margin: 6px 0 0; line-height: 1.6; word-break: break-word; }@media (max-width: 840px) { .settings-shell, .settings-grid { grid-template-columns: 1fr; }.settings-rail { position: static; } }
</style>
